...
from LoRA_Sec.task import get_params, get_model
...
class FLTrustStrategy(_SameRoundForgeMixin, _PoisonedFLForgeMixin, DeterministicClientSelectionMixin, FedAvg):
    ...
    def aggregate_fit(
        ...
        client_updates: List[Tuple[ClientProxy, List[np.ndarray]]] = [
            (client, parameters_to_ndarrays(fit_res.parameters)) for client, fit_res in results
        ]

        # Compute cosine similarity of each client update with the server_update
        similarities = []
        for client, weights in client_updates:
            sim = self._cosine_similarity(self._flatten(weights), self._flatten(server_update))

            similarities.append(sim)

        # Normalize similarities to use as weights
        similarities = np.maximum(similarities, 0)  # Eliminate negative sims
        if np.sum(similarities) == 0:
            similarities = np.ones_like(similarities)
        weights = similarities / np.sum(similarities)

        # Weighted aggregation
        num_layers = len(server_update)
        aggregated = []
        for layer in range(num_layers):
            agg_layer = sum(
                w * client_weights[layer] for w, (_, client_weights) in zip(weights, client_updates)
            )
    ...
    def get_server_trusted_update(self, model_name: str, dataset_name: str, num_samples: int = 500, seed: int = 42, num_labels: int = 2):
        """Train on a small trusted dataset to compute the server-side trusted update."""
        # Load model
        model = get_model(model_name, num_labels)  # or context-configured labels
        model.train()
        ...
        # Load small dataset deterministically
